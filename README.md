# Salut, moi c'est Carl 👋

**Étudiant ingénieur en Robotique** à [Polytech Dijon](https://www.polytech-dijon.fr/) 🇫🇷

Je conçois des robots qui bougent, des modèles qui apprennent et des mécanismes qui marchent (littéralement — demandez à mon Strandbeest). Actuellement en 1ère année du cycle ingénieur, je m'intéresse à la **robotique industrielle**, aux **véhicules autonomes** et à l'**intelligence artificielle appliquée**.

---

### 🛠️ Stack technique

| Domaine | Technologies |
|---|---|
| **Programmation** | Python · C++ |
| **Robotique** | ROS 2 · Gazebo · OpenCV |
| **Machine Learning** | TensorFlow · Scikit-learn |
| **CAO / Fabrication** | SolidWorks · Impression 3D |
| **Outils** | Git · Linux · VS Code |

---

### 🚀 Projets phares

| Projet | Description | Stack |
|---|---|---|
| 🤖 [**autonomous-robot-simulation**](lien-vers-repo) | Simulation d'un robot naviguant de manière autonome sur une carte avec détection d'obstacles | C++ · OpenCV |
| 📄 [**pdf-document-classifier**](lien-vers-repo) | Application de classification automatique de documents PDF par Machine Learning | Python · TensorFlow · Scikit-learn |
| 🦿 [**strandbeest-cad-design**](lien-vers-repo) | Conception et fabrication d'un mécanisme de Strandbeest (Theo Jansen) — 100% mécanique, 0% électronique | SolidWorks · Impression 3D |

---

### 🏆 Hackathons & compétitions

- 🥇 **Hackathon ML — EEIA 2024** (sponsorisé par TotalEnergies) — Prédiction multi-cibles à partir de 14 variables numériques
- ⚡ **Hackathon Arduino Days — Sèmè City 2024** — Prototypage électronique et programmation embarquée
- 💡 **Hackathon FRIARE 2025** — Innovation technologique
- 🎓 **École d'Été sur l'IA (EEIA) 2024** — Bénin Excellence

---

### 📬 Me contacter

[![Email](https://img.shields.io/badge/Email-carlmensah01%40gmail.com-blue?style=flat-square&logo=gmail&logoColor=white)](mailto:carlmensah01@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Carl%20Mensah-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](lien-vers-linkedin)

---

*Actuellement à la recherche d'un stage de 2 mois en robotique (juin–août 2025)* 🔍
